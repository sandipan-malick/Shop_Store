    ...
    <>
        ...
        <div className="container flex flex-wrap items-center justify-between p-3 mx-auto">
          ...
          <div className={`w-full md:w-auto md:flex md:items-center gap-3 ${menuOpen ? "block" : "hidden"}`}>
            <div className="flex flex-col gap-2 mt-3 md:flex-row md:mt-0">
              ...
              <button onClick={() => setShowAddPopup(true)} className="px-4 py-2 bg-green-600 rounded hover:bg-green-700">
                Add Product
              </button>
            </div>
      ...
      {showAddPopup && (
        <div className="fixed inset-0 flex items-center justify-center p-2 bg-black bg-opacity-50">
          <div className="w-full max-w-md p-6 text-black bg-white rounded shadow-lg">
            <h2 className="mb-4 text-xl font-bold">{form._id ? "Edit Product" : "Add New Product"}</h2>
            <form onSubmit={handleSaveProduct} className="space-y-3">
              <input type="text" name="productName" placeholder="Product Name" value={form.productName} onChange={handleChange} className="w-full p-2 border rounded outline-none" required />
              <input type="number" name="productPrice" placeholder="Product Price" value={form.productPrice} onChange={handleChange} className="w-full p-2 border rounded outline-none" min="0" required />
              <input type="number" name="productSellPrice" placeholder="Product Selling Price" value={form.productSellPrice} onChange={handleChange} className="w-full p-2 border rounded outline-none" min="0" required />
              <input type="number" name="quantity" placeholder="Quantity" value={form.quantity} onChange={handleChange} className="w-full p-2 border rounded outline-none" min="0" />
              <textarea name="productDescription" placeholder="Product Description" value={form.productDescription} onChange={handleChange} className="w-full p-2 border rounded outline-none" rows="3"></textarea>
              <div className="flex justify-end gap-3">
                ...
                <button type="submit" className="px-4 py-2 text-white bg-blue-600 rounded outline-none">
                  Save
                </button>
              </div>
            </form>
                  ...
                  >
                    ...
                    {decreaseForItem === item._id && (
                      <div className="w-full p-3 mt-3 bg-yellow-100 border border-yellow-300 rounded outline-none">
                        <label className="block mb-1 text-sm font-semibold text-yellow-900">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <button
                            onClick={() => applyDecrease(item._id, item.quantity)}
                            disabled={!decreaseValue || decreaseValue <= 0 || decreaseValue > item.quantity}
                            className="px-3 py-1 text-white bg-yellow-600 rounded outline-none disabled:opacity-50"
                          >
                            Apply
                          </button>
                          <button
                            onClick={() => setDecreaseForItem(null)}
                            className="px-3 py-1 text-yellow-900 bg-yellow-200 rounded outline-none"
                          >
                            Cancel
                          </button>
                          <span className="ml-auto text-xs text-yellow-800">Current stock: {item.quantity}</span>
                        </div>
                      </div>
                    )}
                    ...
                    {updateForItem === item._id && (
                      <div className="w-full p-3 mt-3 bg-blue-100 border border-blue-300 rounded">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            onClick={() => setUpdateForItem(null)}
                            className="px-3 py-1 text-blue-900 bg-blue-200 rounded"
                          >
                            Cancel
                          </button>
                        </div>
